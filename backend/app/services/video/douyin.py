"""抖音视频解析与下载模块 — 基于公开 API，无需 Cookie"""
...
class DouyinParser:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    raise ValueError(f"链接解析失败: {e}") from e
    ...
    def _fetch_via_share_page(self, video_id: str, resolved_url: str) -> dict:
        ...
        share_url = resolved_url if "iesdouyin.com" in (parsed.netloc or "") else (
        ...
        for node in router_data.get("loaderData", {}).values():
