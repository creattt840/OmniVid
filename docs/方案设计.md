...
POST /api/subtitles/translate  → 字幕翻译下载（6 语言，VIP）
```
...
GET  /api/auth/me        → { id, email, is_vip, vip_expires_at, ai_usage_today }
```

### 3.11 Stripe 支付（第 6 阶段）

```
POST /api/billing/checkout  → { checkout_url }（需登录）
POST /api/billing/webhook   → Stripe 回调（checkout.session.completed）
```

AI 分析门控：`POST /api/analyze` 需登录；免费用户每日 3 次；翻译/改写需 VIP。
...
7. **PricingSection** — 免费 vs VIP，Stripe 支付入口
8. **Footer** — 免责声明
...
> 第 5–6 阶段（用户登录 + Stripe VIP）已完成，详见 `docs/阶段总结-用户登录与VIP会员.md`。
