...
│  │    ai/             DeepSeek 摘要 / Whisper 转写 / 字幕 / 质量门控   │  │
...
→ 提交前校验是否为视频平台链接（非视频 URL 返回 400 友好提示）
...
→ 与 AI 分析共用 transcript_quality 质量门控
→ FileResponse（字幕文件）
→ 响应头 X-Transcript-Source: subtitle | whisper
```

前端：仅当平台有字幕轨、或 AI 转录成功时显示「下载字幕」按钮；无对白/无法转录的视频不展示该入口。
...
POST /api/analyze
  ...
  → transcript_quality.assert_transcript_quality() 校验（URL 路径；本地上传外挂字幕豁免）
  → 质量不足时 400：「未检测到足够的人声内容…」
```
